...
def eye_train(model, train_loader, optimizer, epoch, device, params):
    print("In eye_train")
    model.train()
...
def chex_train(model, train_loader, optimizer, epoch, device, params:
    print("In chex_train")
...
def wrn_train(params):
    ...
    for epoch in range(EPOCHS):
        if params['dataset'] == 'chexpert':
            chex_train(model, train_loader, optimizer, epoch + 1, device, params)
        elif params['dataset'] == 'eyepacs':
            eye_train(model, train_loader, optimizer, epoch + 1, device, params)
